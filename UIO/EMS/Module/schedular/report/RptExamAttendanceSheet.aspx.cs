...
{
    public partial class RptExamAttendanceSheet : BasePage
    {
        ...
        protected void buttonView_Click(object sender, EventArgs e)
        {
            ...
            LoadExamAttandanceSheet(acaCalId, examSetId, dayId, timeSlotId, roomId);           
        }

        private void LoadExamAttandanceSheet(int acaCalId, int examSetId, int dayId, int timeSlotId, int roomId)
        {            
            ...
            ReportParameter p1 = new ReportParameter("RoomNo", room);
            ...
            List<rExamAttendanceSheet> list = ExamScheduleManager.GetExamAttendaceListByRoom(acaCalId, examSetId, dayId, timeSlotId, roomId);

            try
            {
                if (list.Count != 0)
                {
                    ExamAttandanceSheet.LocalReport.ReportPath = Server.MapPath("~/miu/schedular/report/RptExamAttendanceSheet.rdlc");
                    this.ExamAttandanceSheet.LocalReport.SetParameters(new ReportParameter[] { p1, p2, p3, p4, p5, p6 });
                    ReportDataSource rds = new ReportDataSource("RptAttendanceSheet", list);

                    ExamAttandanceSheet.LocalReport.DataSources.Clear();
                    ExamAttandanceSheet.LocalReport.DataSources.Add(rds);
                    lblMessage.Text = "";
                }
                else
                {
        ...
        protected void buttonPrint_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (list.Count != 0)
                {
                    ...
                    byte[] bytes = ExamAttandanceSheet.LocalReport.Render("PDF", null, out mimeType, out encoding, out filenameExtension, out streamids, out warnings);

                    using (FileStream fs = new FileStream(Server.MapPath("~/Upload/ReportPDF/" + "ExamAttandanceSheet" + ".pdf"), FileMode.Create))
                    {
                        fs.Write(bytes, 0, bytes.Length);
                    }

                    string path = Server.MapPath("~/Upload/ReportPDF/" + "ExamAttandanceSheet" + ".pdf");
